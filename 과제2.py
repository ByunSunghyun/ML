...
# print("df_train cpy insa sum=", df_train_copy.isna().sum())
...
titanic_df['Age'].fillna(titanic_df['Age'].mean(),
                         inplace=True)  # Age의 평균값으로 결측치 대체
...
sns.barplot(x='Pclass', y='Survived', hue='Sex',
            data=titanic_df)  # hue는 x를 더욱 세부적으로 나누기 위한 특성
...
def drop_features(df):
    df.drop(['PassengerId', 'Name', 'Ticket'],
            axis=1, inplace=True)  # 모두 단순 구분을 위한 컬럼들
    return df
...
print('DecisionTreeClassifier 정확도 : {0:.4f}'.format(
    accuracy_score(y_test, dt_pred)))

# RandomForestClassifier 학습/예측/평가
rf_clf.fit(X_train, y_train)
rf_pred = rf_clf.predict(X_test)
print('RandomForestClassifier 정확도 : {0:.4f}'.format(
